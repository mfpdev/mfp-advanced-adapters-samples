...
/**
 * Adds the JSON representation as returned by getJSON to an event.
 ...
 * The WLDeviceContext instance received by [WLTriggerCallback execute:] would add its data to a dynamic event object which would then be passed to [WLClient transmitEvent:].
 * If the JSON representation is <code>null</code>, then JSONObject.NULL is added.
